...
// Fungsi untuk toggle field balita berdasarkan kategori
function toggleBalitaFields(laporanIndex, kategori) {
...
function showImagePreview(laporanIndex, fileName) {
  ...
  if (compressedImageBlobs[laporanIndex]) {
    ...
    removeBtn.onclick = () => removePhoto(laporanIndex);
...
function addEventListeners(laporanIndex) {
  ...
  kategoriSelect.addEventListener("change", function () {
    toggleBalitaFields(laporanIndex, this.value);
  });
...
function getAllLaporanData() {
  ...
  for (let i = 1; i <= jumlahLaporan; i++) {
    ...
    // Tambahkan data balita jika kategori adalah balita
    if (kategori === "balita") {
...
function createDataParagraphs(formData) {
  ...
  // Tambahkan LILA dan LIKA jika kategori balita
